    ...
    def update_trade(self, ticket: int, current_price: float) -> Optional[Tuple[float, str]]:
        ...
        with self.lock:
            ...
            if not trade['breakeven_moved']:
                ...
                if current_profit_pips >= config.BREAKEVEN_AT_PIPS:
                    should_move_breakeven = True
                    reason = f"Breakeven at {current_profit_pips:.1f} pips"
                ...
                if should_move_breakeven:
                    ...
                    if (direction == 'buy' and new_sl > current_sl) or \
                       (direction == 'sell' and new_sl < current_sl):
                        ...
                        trade['last_move_pips'] = current_profit_pips
                        trade['last_move_profit_usd'] = current_profit_usd
                        logger.logger.info(f"Moving to breakeven: {reason}")
                        return (new_sl, f"breakeven_{reason}")
            
            target_pips = 0
            for pips, threshold_price in trade['pip_thresholds']:
            ...
            target_profit_usd = 0
            if current_profit_usd >= config.MIN_PROFIT_FOR_MOVE:
                profit_increments = range(7, int(current_profit_usd) + 7, 7)
                for increment in profit_increments:
                    if current_profit_usd >= increment:
                        target_profit_usd = increment
            
            move_triggered = False
            move_reason = ""
            
            if target_pips > trade['last_move_pips']:
                move_triggered = True
                move_reason = f"{target_pips}_pips"
            ...
            if move_triggered:
                if not trade['breakeven_moved']:
                    new_sl = entry_price
                    move_reason = f"breakeven_{move_reason}"
                else:
                    if direction == 'buy':
                        new_sl = current_price - (70 * trade['pip_value'])
                ...
                if (direction == 'buy' and new_sl > current_sl) or \
                   (direction == 'sell' and new_sl < current_sl):
                    trade['current_sl'] = new_sl
                    trade['last_move_pips'] = max(trade['last_move_pips'], target_pips)
                    trade['last_move_profit_usd'] = max(trade['last_move_profit_usd'], target_profit_usd)
                    logger.logger.info(f"Moving SL: {move_reason}, new SL: {new_sl:.5f} (profit: {current_profit_pips:.1f} pips, ${current_profit_usd:.2f})")
...
def send_trade_safe(symbol: str, direction: str, lot: float, sl_price: float, tp_price: float, reason: str, confluence_score: int = 0, fvg_signal: bool = False):
    ...
    try:
        ...
        ticket = getattr(res, 'order', 0)
        if ticket > 0:
            trade_tracker.add_trade(ticket, symbol, direction, price, sl_price, tp_price, confluence_score, fvg_signal)
